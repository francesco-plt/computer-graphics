...
private:
	...
	{
		auto app = reinterpret_cast<Assignment19 *>(glfwGetWindowUserPointer(window));
	...
	void createModelMesh(ModelData &MD, VertexDescriptor &VD)
	{
		...
		MD.indices.resize(M1_indices.size() + M2_indices.size() + 3 * (M1_InternalVertices.size() + M2_InternalVertices.size()));
