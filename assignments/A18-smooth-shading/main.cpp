...
private:
	...
	{
		auto app = reinterpret_cast<Assignment18 *>(glfwGetWindowUserPointer(window));
	...
	void createModelMesh(ModelData &MD, VertexDescriptor &VD)
	{
		...
		MD.indices.resize(M1_indices.size() + M2_indices.size() + M3_indices.size() + M4_indices.size() + 3 * (M1_InternalVertices.size() + M2_InternalVertices.size() + M3_InternalVertices.size() + M4_InternalVertices.size()));
